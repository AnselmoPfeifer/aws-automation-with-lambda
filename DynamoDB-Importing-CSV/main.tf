...
resource "aws_s3_bucket" "s3_bucket_csv" {
  bucket  = "aws-lambda-csv-data-${random_string.random.result}"
}
  ...
  policy = <<EOF
{
	...
	"Statement": [
		...
		},
		{
			...
			"Resource": [
              "${aws_s3_bucket.s3_bucket_csv.arn}",
              "${aws_s3_bucket.s3_bucket_csv.arn}/*"
            ]
		},
		{
...
resource "aws_lambda_permission" "lambda_permission" {
  ...
  source_arn = aws_s3_bucket.s3_bucket_csv.arn
}

resource "aws_s3_bucket_notification" "bucket_notification" {
  bucket = aws_s3_bucket.s3_bucket_csv.id
