...
EMAIL_FROM = 'all@mailcom'  # Use a verified SES email address
EMAIL_TO = 'all@mailcom'
MAX_AGE = 90
...
def lambda_handler(event, context):
    ...
    for response in paginator.paginate():
        ...
        for user in response['Users']:
            ...
            print(f"INFO: starting checking access key for user: {user['UserName']}")
            ...
            for access_key in res['AccessKeyMetadata']:
                ...
                # Expire the key
                print(f'INFO: Key {access_key_id} for user {username} is expired (age={age} days).')
...
def send_email_report(email_to, username, age, access_key_id):
    data = (f'NFO: access key {access_key_id} belonging to user {username} has been '
